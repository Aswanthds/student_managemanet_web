...
import 'package:student_management_web/widgets/add_student_form.dart';
...
class _AddStudentDialogState extends State<AddStudentDialog> {
  ...
  String formatFileName(String filename) {
    ...
    if (parts.length > 1) {
      ...
      return nameWithoutExtension; // Output: aswanth
    } else {
  ...
  Widget build(BuildContext context) {
    Widget imageWidget = isImageSelected
        ? Row(
            ...
            children: [
              Image.network(
                'https://firebasestorage.googleapis.com/v0/b/studentmanagemnetweb.appspot.com/o/StudentImages%2F$filename?alt=media&token=2df4a60b-1f89-4710-8d1a-2bd00e4380b9',
                height: 80,
                width: 80,
              ),
              IconButton(
                onPressed: () async {
                  if (isImageSelected) {
                    await FirebaseFunctions.deleteImageFromStorage(
                        imageFile); // Delete the image from Firebase Storage
                  }
                  setState(() {
                    isImageSelected = false;
                    imageFile = ''; // Clear the image file name
                  });
                },
